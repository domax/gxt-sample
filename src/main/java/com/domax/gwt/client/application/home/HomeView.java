...
public class HomeView extends ViewImpl implements HomePresenter.MyView {
  ...
  ThemeStyles.Styles themeStyle = ThemeStyles.get().style();
  ...
  public void setHomeInfo(HomeInfo homeInfo) {
    ...
    Optional.ofNullable(homeInfo.getLines())
        ...
        .forEach(
            line ->
                builder.appendHtmlConstant("<p>").appendEscaped(line).appendHtmlConstant("</p>"));
    lines.setHTML(builder.toSafeHtml());

    timestamp.setText(
        Optional.ofNullable(homeInfo.getTimestamp()).map(Date::toString).orElse("No timestamp"));
  }
}
